...
$(function() {
	...
	$("#renderButton").click(refreshPreview);
			...
			if(big_frag[0] == 0){
				...
				for(var k=0; k < array2.length; k++) {
					...
					if(small_frag[0]==0){
						...
						node = renderFormula(small_frag[1]);
						node.tex = small_frag[1];
					}
					p_elt.appendChild(node)
			   }
			} else {
				img_elt = renderFormula('\\displaystyle ' + big_frag[1]); // See Heilmann's "JavaScript with DOM .. " page 127.
